...
> :warning: **Using an `open` policy with global_deny_list can be tricky**, because the global_deny_list will only block specific *domains*, not entire *destinations*. For example, if `www.example.com` is in the global_deny_list but the IP address that it resolves to is not, roles with an `open` policy will still be able to access the destination by using its IP address directly. For this reason, **we recommend using allowlists instead of denylists, whenever it is possible to do so.**
